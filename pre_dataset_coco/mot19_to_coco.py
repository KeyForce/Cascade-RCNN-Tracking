...
root_path = '/mnt/liucen/MOT_challenge/MOT_19/'
# 检测框的ID起始值
...
def get_size(label_txt, frame):
    p = 'img1/{:>06d}.jpg'.format(int(frame))
    pic = root_path + p
    im = Image.open(pic)
...
def convert(label_list, json_file):
    ...
    for label_txt in label_list:                                        #循环标签列表
        ...
        for frame in tqdm(txt_dir.keys()):
            ...
            #复制图片
            shutil.copyfile(picfile, '/mnt/liucen/MOT_det/coco_mot19/train2017/'+filename)
...
def MOT19_TO_COCO():
    #设置转换的label文件
    label_list = ['det/det.txt']
    json_file = '/mnt/liucen/MOT_det/coco_mot19/annotations/instances_train2017.json'
