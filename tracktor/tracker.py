...
class Tracker:
	...
	def __init__(self, reid_network, tracker_cfg):
	...
	def reid(self, boxes, cls_conf, img, new_det_pos, new_det_scores):
		...
		if self.do_reid:
			...
			if len(self.inactive_tracks) >= 1:
				...
				dist_mat = dist_mat * iou_mask.cuda().float() + iou_neg_mask.cuda().float() * 1000
	...
	def get_appearances(self, img):
		"""Uses the siamese CNN to get the features for all active tracks."""
		new_features = self.reid_network.test_rois(img, self.get_pos()).data
		return new_features
	...
	def step(self, boxes, cls_conf, img):
		...
		if boxes.nelement() > 0:
			...
			inds = torch.gt(cls_conf, self.detection_person_thresh).nonzero().view(-1)
		else:
		...
		if len(self.tracks):
			...
			# regress
			person_scores = self.regress_tracks(boxes, cls_conf, img)

			if len(self.tracks):
				# create nms input

				# nms here if tracks overlap
				keep = nms(self.get_pos(), person_scores.cpu(), self.regression_nms_thresh)

				self.tracks_to_inactive([self.tracks[i] for i in list(range(len(self.tracks))) if i not in keep])

				if keep.nelement() > 0:
					if self.do_reid:
						new_features = self.get_appearances(img)
		...
		if det_pos.nelement() > 0:
			...
			new_det_pos, new_det_scores, new_det_features = self.reid(boxes, cls_conf, img, new_det_pos, new_det_scores)
		...
		for t in self.tracks:
			...
			id_output.append(t.id)
			box_output.append(t.pos[0].cpu().numpy())
